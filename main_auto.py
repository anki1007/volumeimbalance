"""
ChartVision Pro X - Institutional Trading Platform
Backend API with Gemini AI Integration & Multi-Broker Support
Supports: IIFL Blaze (XTS), Zerodha, Upstox, FYERS with Auto-Login (TOTP) | Dhan with Access Token auth
"""
...
load_dotenv("/home/ubuntu/ChartVisionProX/.env")

GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")

BROKER_USER_ID = os.getenv("BROKER_USER_ID")
BROKER_PASSWORD = os.getenv("BROKER_PASSWORD")
BROKER_TOTP_SECRET = os.getenv("BROKER_TOTP_SECRET")

INTERACTIVE_API_KEY = os.getenv("INTERACTIVE_API_KEY")
INTERACTIVE_API_SECRET = os.getenv("INTERACTIVE_API_SECRET")

MARKET_API_KEY = os.getenv("MARKET_API_KEY")
MARKET_API_SECRET = os.getenv("MARKET_API_SECRET")
...
class RiskManager:
    ...
    def check_duplicate(self, order: OrderRequest) -> tuple:
        key = f"{order.symbol}_{order.transaction_type.value}_{order.quantity}_{order.order_type.value}_{order.price}_{order.trigger_price}"
        now = time.time()
...
async def lifespan(app: FastAPI):
    ...
    if GEMINI_API_KEY:
        try:
            app_state["gemini_client"] = GeminiClient(GEMINI_API_KEY)
            logger.info("✅ Gemini auto configured from .env")
        except Exception as e:
            logger.error(f"Gemini auto config failed: {e}")
    ...
    async def _auto_broker_login():
        try:
            logger.info("🔐 Attempting auto broker login...")

            creds = BrokerCredentials(
                broker=BrokerType.IIFL_BLAZE,
                api_key=INTERACTIVE_API_KEY,
                api_secret=INTERACTIVE_API_SECRET,
                user_id=BROKER_USER_ID,
                password=BROKER_PASSWORD,
                totp_secret=BROKER_TOTP_SECRET,
                market_api_key=MARKET_API_KEY,
                market_secret_key=MARKET_API_SECRET
            )

            sid = app_state["session_manager"].create_session("AUTO_SYSTEM")

            ok = await app_state["session_manager"].connect_broker(sid, creds)

            if ok:
                logger.info("✅ AUTO BROKER LOGIN SUCCESS")
            else:
                logger.error("❌ AUTO BROKER LOGIN FAILED")

        except Exception as e:
            logger.error(f"Auto broker login error: {e}")

    await _auto_broker_login()
    yield
...
# ---------------- IIFL MARKET DATA INTEGRATION ----------------
import websockets
import json

async def connect_iifl_market(session_manager):
    try:
        logger.info("📡 Connecting IIFL Market Feed...")

        url = "https://ttblaze.iifl.com/apimarketdata/auth/login"

        payload = {
            "appKey": session_manager.market_api_key,
            "secretKey": session_manager.market_api_secret,
            "source": "WebAPI"
        }
        ...
        session_manager.market_token = data.get("result", {}).get("token")

        if not session_manager.market_token:
            logger.warning("⚠ Market token not received")
            return

        logger.info("✅ IIFL Market login success")

        # Start websocket
        asyncio.create_task(start_iifl_market_ws(session_manager))

    except Exception as e:
        logger.error(f"Market connection error: {e}")


async def start_iifl_market_ws(session_manager):
    try:
        ws_url = "wss://ttblaze.iifl.com/apimarketdata/socket"

        async with websockets.connect(ws_url) as ws:
            logger.info("📡 IIFL Market WebSocket connected")

            auth_msg = {
                "token": session_manager.market_token
            }
            ...
            while True:
                ...
                # Store last price snapshot (basic)
                session_manager.last_market_tick = data
